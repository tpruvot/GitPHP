...
<div class="page_path">
	{if $pathobject}
		...
		{if $pathobject instanceof GitPHP_Blob}
			{if $target == 'blobplain'}
				<a href="{geturl project=$project action=blob_plain hash=$pathobject hashbase=$pathobjectcommit file=$pathobject->GetPath()}"><strong>{$pathobject->GetName()|escape}</strong></a>
			{elseif $target == 'blob'}
