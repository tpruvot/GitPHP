...
{block name=main}
 ...
 <div class="page_nav">
   ...
   {if $mark}
     {t}selected{/t} &sdot;
     <a href="{$SCRIPT_NAME}?p={$project->GetProject()|urlencode}&amp;a=commit&amp;h={$mark->GetHash()}" class="list commitTip" {if strlen($mark->GetTitle()) > 80}title="{$mark->GetTitle()|htmlspecialchars}"{/if}><strong>{$mark->GetTitle(80)}</strong></a>
     &sdot;
