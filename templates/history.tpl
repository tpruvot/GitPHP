 ...
 <table cellspacing="0">
   ...
   {foreach from=$blob->GetHistory() item=historyitem}
     ...
     <tr class="{cycle values="light,dark"}">
       ...
       <td><a href="{$SCRIPT_NAME}?p={$project->GetProject('f')}&amp;a=commit&amp;h={$historycommit->GetHash()}" class="list commitTip" {if strlen($historycommit->GetTitle()) > 80}title="{$historycommit->GetTitle()}"{/if}><strong>{$historycommit->GetTitle(80)}</strong></a>
