...
class GitPHP_CommitLoad_BaseTest extends PHPUnit_Framework_TestCase
{
	public function testLoadContainingTag()
	{
		...
		$exemock->expects($this->exactly(2))->method('Execute')->with($this->equalTo(GITPHP_TEST_PROJECTROOT . '/testrepo.git'), $this->equalTo('name-rev'))->will($this->onConsecutiveCalls($data, $data2));
